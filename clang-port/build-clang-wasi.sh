...
cd llvm-project 
cmake -S llvm -B build -G Ninja -DCMAKE_BUILD_TYPE=MinSizeRel -DLLVM_ENABLE_PROJECTS="clang" -DCMAKE_INSTALL_PREFIX=${INST} -DLLVM_TARGETS_TO_BUILD=X86 -DCMAKE_CROSSCOMPILING:BOOL=ON -DLLVM_ENABLE_THREADS:BOOL=OFF -DLLVM_TABLEGEN=${WASI_SDK}/bin/llvm-tblgen -DCLANG_TABLEGEN=${WASI_SDK}/bin/clang-tblgen -DLLVM_INCLUDE_TESTS:BOOL=OFF -DLLVM_INCLUDE_EXAMPLES:BOOL=OFF -DLLVM_INCLUDE_UTILS:BOOL=OFF -DLLVM_INCLUDE_BENCHMARKS:BOOL=OFF -DLIBCLANG_BUILD_STATIC:BOOL=ON
